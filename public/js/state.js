const state = {
  store: {
    branches: [],
    currentBranch: "carrizal",
    currentBranchLabel: "Carrizal",
    shifts: ["Manana", "Tarde"],
    timezone: "America/Mexico_City",
  },
  profile: {
    businessName: "Merxalia POS",
    shortName: "Merxalia",
    slug: "merxalia-pos",
    currencyCode: "MXN",
    locale: "es-MX",
    timezone: "America/Mexico_City",
    ticketPrefix: "RIN",
    templateKey: "cremeria",
    branding: {},
    visibleTexts: {},
  },
  enabledModules: [],
  support: {
    label: "Soporte",
    whatsappUrl: "",
    configured: false,
  },
  adminCapabilities: [],
  categories: [],
  units: [],
  productAttributeDefinitions: [],
  businessTemplates: [],
  products: [],
  lowStock: [],
  recentSales: [],
  recentActivity: [],
  salesByHour: [],
  shiftSummary: [],
  summary: {
    revenueToday: 0,
    ticketsToday: 0,
    averageTicket: 0,
    unitsSoldToday: 0,
    catalogSize: 0,
    inventoryValue: 0,
    inventorySaleValue: 0,
    inventoryCostValue: 0,
    missingCostProductsCount: 0,
    negativeStockProductsCount: 0,
    averageMarginPercent: 0,
    lowStockCount: 0,
    topProduct: null,
  },
  selectedCategory: "all",
  visibleProductLimit: PRODUCT_RENDER_BATCH,
  currentProduct: null,
  currentProductCartIndex: null,
  cart: [],
  routeCartEditor: {
    index: null,
    editSource: "quantity",
  },
  paymentMethod: "Efectivo",
  moneyInput: "0",
  paymentReceivedMethod: "Efectivo",
  paymentCustomerName: "",
  paymentNote: "",
  socket: null,
  online: typeof navigator !== "undefined" ? navigator.onLine : true,
  pendingQueue: [],
  offlineSales: [],
  offlineReceivablePayments: [],
  receivablesCache: {
    branches: {},
  },
  syncingQueue: false,
  quickImport: {
    mode: "receive", // "receive" (entrada), "return" (salida) o "count" (conteo fisico)
    items: [],
    index: 0,
    lastVisitedIndex: 0,
    loading: false,
    saving: false,
    supplierName: "",
    search: "",
    filteredIndexes: [],
    draftsByProductId: {},
    savedByProductId: {},
    skippedByProductId: {},
  },
  merchandise: {
    items: [],
    supplierName: "",
    notes: "",
    search: "",
    quickQuantities: {},
    quickTotals: {},
    saving: false,
    loadingMyRequests: false,
    myRequests: [],
    currentProduct: null,
    currentMode: "receive",
    currentQuantity: "",
    detailRequest: null,
    detailLoading: false,
    detailProcessing: false,
    detailAdminMode: false,
    detailRejectReason: "",
  },
  mobileApprovals: {
    active: false,
    accessState: "idle",
    statusMessage: "",
    requests: [],
    listLoading: false,
    detailLoading: false,
    processing: false,
    selectedRequestId: null,
    detail: null,
    rejectReason: "",
    confirmApprove: false,
  },
  receivables: {
    loading: false,
    search: "",
    customers: [],
    source: "idle",
    selectedCustomerKey: "",
    detailLoading: false,
    detailCustomer: null,
    detailSource: "idle",
    paymentLoading: false,
    paymentSaleId: null,
    paymentAmount: "",
    paymentMethod: "Efectivo",
    paymentNote: "",
  },
  register: {
    mode: "start",
    summary: null,
    events: [],  // Eventos de caja guardados offline
    loading: false,
    saving: false,
    amountInput: "",
    withdrawInput: "0",
    note: "",
  },
  cashierBlindAudit: {
    prompt: null,
    draftItems: {},
    previewByItemId: {},
    loading: false,
    saving: false,
    previewing: false,
  },
  detailViewer: {
    loading: false,
    kind: "",
    detail: null,
  },
  admin: {
    branch: "all",
    activeSection: "overview",
    inventoryExpanded: false,
    inventoryMode: "movement",
    inventorySearch: "",
    inventoryFilter: "all",
    productEditor: {
      productId: null,
      branch: "",
    },
    inventoryProducts: [],
    inventoryPage: {
      limit: 120,
      offset: 0,
      total: 0,
      hasMore: false,
      loading: false,
    },
    inventoryComparison: null,
    onboarding: {
      file: null,
      preview: null,
      mapping: {},
      loading: false,
      applying: false,
      status: "",
    },
    loading: false,
    metricsLoading: false,
    metrics: null,
    profitability: null,
    profitabilityPeriod: "today",
    profitabilityDateKey: "",
    subscription: null,
    subscriptionControl: null,
    subscriptionPayments: [],
    supportHealth: null,
    controlPlaneConfigSyncedAt: 0,
    controlPlaneConfigSyncing: false,
    controlPlaneHealthReportedAt: 0,
    controlPlaneHealthReporting: false,
    backupsStatus: null,
    pollTimerId: null,
    username: "",
    csrfToken: "",
    sessionExpiresAt: null,
    configured: false,
    setupAllowed: false,
    authenticated: false,
    capabilitiesResolved: false,
    branches: [],
    snapshot: null,
    auditLogs: [],
    cashiers: [],
    merchandiseRequests: [],
    weightedAudit: {
      sessions: [],
      currentSession: null,
      loading: false,
      saving: false,
      dateKey: "",
      shift: "Tarde",
      search: "",
      showPendingOnly: false,
      showIncidentsOnly: false,
      draftItems: {},
      notesDraft: "",
    },
    periodClosures: {
      periodType: "week",
      dateKey: "",
      preview: null,
      closures: [],
      currentClosure: null,
      selectedClosureId: null,
      loading: false,
      detailLoading: false,
      saving: false,
      notesDraft: "",
    },
    editorData: {
      sales: [],
      registerEvents: [],
      inventoryMovements: [],
    },
    receivableDuplicates: [],
    receivableDuplicateSearch: "",
    receivableDuplicatesLoading: false,
    receivableDuplicateMerging: false,
    productDuplicates: [],
    productDuplicateSearch: "",
    productDuplicatesLoading: false,
    productDuplicateMerging: false,
    workspaceLoadedAt: {
      snapshot: 0,
      editorData: 0,
      receivableDuplicates: 0,
      productDuplicates: 0,
      auditLogs: 0,
      branches: 0,
      cashiers: 0,
      requests: 0,
      config: 0,
      weightedAudit: 0,
      periodClosures: 0,
    },
    configLoading: false,
    configSaving: false,
    configDirty: false,
    configEditRevision: 0,
    configMutationGeneration: 0,
    configLoadRequestId: 0,
    configDirtyRevisions: {
      businessProfile: {},
      settings: {},
      enabledModules: 0,
    },
    brandingLogo: {
      file: null,
      previewUrl: "",
      uploading: false,
      removing: false,
    },
  },
  adminAuth: {
    mode: "login",
    loading: false,
  },
  owner: {
    username: "",
    csrfToken: "",
    sessionExpiresAt: null,
    configured: false,
    setupAllowed: false,
    authenticated: false,
    loading: false,
    saving: false,
    runtimeConfigSaving: false,
    runtimeConfigSyncing: false,
    accessLoaded: false,
    availableModules: [],
    adminSections: [],
    templates: [],
    operationGuide: null,
    templateReset: {
      applying: false,
      selectedTemplateKey: "",
      businessName: "",
      slug: "",
      workbookPath: "",
      confirmText: "",
      confirmReset: false,
    },
  },
  ownerAuth: {
    mode: "login",
    loading: false,
    revealClicks: 0,
    revealWindowStartedAt: 0,
  },
  adminEditor: {
    kind: "",
    id: null,
    loading: false,
    saving: false,
    detail: null,
  },
  cashier: {
    id: null,
    token: "",
    name: "",
    branch: "",
    expiresAt: "",
    authenticated: false,
  },
  cashierAuth: {
    loading: false,
  },
  performance: {
    productsRenderMs: 0,
    snapshotRenderMs: 0,
    renderedProductCount: 0,
    routeQuickAddMs: 0,
    routeQuickAddCount: 0,
    routeCartEditorOpenMs: 0,
    routeCartEditorOpenCount: 0,
  },
  ui: {
    routeMode: false,
    routeRegisterCollapsed: true,
    routeCartMotionIndex: null,
    scannerMode: false,
    scannerLastCode: "",
    scannerLastStatus: "idle",
  },
};

// Referencias a elementos del DOM
const refs = {};
let offlineDbPromise = null;
